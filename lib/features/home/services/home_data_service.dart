...
class HomeDataService {
  ...
  static List<CriticalAlert> getMockCriticalAlerts(String? homeId, {List<Map<String, dynamic>> realAssets = const []}) {
    ...
    for (final asset in realAssets) {
      ...
      if (healthScore < 5.0) {
        allAlerts.add(CriticalAlert(
          ...
          category: asset['type']?.toString() ?? 'Appliance',
          location: location,
          issue: 'Health score critical (${healthScore.toStringAsFixed(1)}/10) — service recommended',
        ...
        allAlerts.add(CriticalAlert(
          ...
          category: asset['type']?.toString() ?? 'Appliance',
          location: location,
          issue: 'Health score low (${healthScore.toStringAsFixed(1)}/10) — maintenance recommended',
          severity: AlertSeverity.warning,
          alertCategory: AlertCategory.maintenance,
  ...
  static List<ActiveService> getMockActiveServices(String? homeId, {List<Map<String, dynamic>> realAssets = const []}) {
    // If user has no real assets, return empty (new user experience)
    if (realAssets.isEmpty) return [];
  ...
  static List<PendingDelivery> getMockPendingDeliveries(String? homeId, {List<Map<String, dynamic>> realAssets = const []}) {
    // If user has no real assets, return empty (new user experience)
    if (realAssets.isEmpty) return [];
  ...
  // Get summary counts
  static Map<String, int> getHomeSummary(String? homeId, {List<Map<String, dynamic>> realAssets = const []}) {
