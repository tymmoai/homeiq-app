...
class ModelNumberDetector {
  ...
  static String? detectAssetType(String? modelNumber) {
    ...
    final model = modelNumber.trim().toUpperCase().replaceAll(RegExp(r'\s+'), '');
...
class _PrefixRule {
  ...
  const _PrefixRule(this.prefix, this.assetType, this.brand, {this.isRegex = false});
}
...
const List<_PrefixRule> _prefixRules = [
  ...
  //  HISENSE
  // ═══════════════════════════════════════════════════════════════
  _PrefixRule(r'^[0-9]{2}A',  'Television', 'Hisense', isRegex: true),  // e.g. 55A6G
  _PrefixRule(r'^[0-9]{2}U',  'Television', 'Hisense', isRegex: true),  // e.g. 65U8G
  _PrefixRule(r'^[0-9]{2}H',  'Television', 'Hisense', isRegex: true),  // e.g. 50H8G
